<!--************************************
		Banner Start
*************************************-->
<section class="banner haslayout parallax-window hidden-xs" data-parallax="scroll" data-bleed="100" data-speed="0.2" data-image-src="{{ url(dirTemaToko().'fashioner/assets/images/bg-footer.png') }}">
	<div class="container">
		@if(@$blog_category)
		<h1>{{ $blog_category->nama }}</h1>
		@else
		<h1>Blog</h1>
		@endif
	</div>
...
<div class="breadcrumbs haslayout">
	<div class="container">
		<span class="page-title">{{@$blog_category ? $blog_category->nama : 'Blog'}}</span>
		<ol class="breadcrumb">
			<li><a href="{{ url('home') }}">Home</a></li>
			@if(@$blog_category)
			<li><a href="{{ url('blog') }}">Blog</a></li>
			<li class="active">{{ $blog_category->nama }}</li>
			@else
					...
					<aside id="sidebar">
						...
						<div class="widget category">
							...
							<ul>
								...
								<li><a href="{{ blog_category_url($blog_category) }}" {{ Request::url() == blog_category_url($blog_category) ? 'class="active"' : '' }}>{{ $blog_category->nama }}</a></li>
								@endforeach
							</ul>
						</div>
						@endif
